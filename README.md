# Alien Isolation Xenomoforph AI - Godot Recrecreation

This project is a fan-made, non-commercial recreation of the AI behavior seen in Alien Isolation, made for educational purposes only. All assets and references belong to their respective owners.

## Demo Preview

Watch short clips and devlogs:
[@a6xdev on X/Twitter](https://x.com/a6xdev)

## Features Implemented

- Finite State Machine (FSM) for AI Behavior
- Perception System
- Patrol, search and investigation logic
- Player Detection and AI chase bahevaior 
- Custom Debug mode with:
    - Vision cone
    - Path visualization
    - Free Camera
- Full Godot Project open-source

## License & Credits

This project is licensed under the MIT License.
Model and animation used for learning purposes only.

```
Alien: Isolation is © Creative Assembly and SEGA.
This project is not affiliated with, endorsed by, or associated with them.
```

## About the developer
Hi, I'm Aislan --  a game developer from Brazil. I build AI system, gameplay mechanics and some bugs.

- [Youtube](https://www.youtube.com/@a6xdev11)
- [X/Twitter](https://x.com/a6xdev)

## Tools & Assets

- **Godot Engine (4.4.1)**

- [ImGui](https://github.com/pkdawson/imgui-godot)
- [On-Screen Logger](https://github.com/SchimmelSpreu83/On-Screen-Logger)
- [VisionCone3D](https://github.com/Tattomoosa/VisionCone3D)
- [Xenomorph](https://sketchfab.com/3d-models/alien-isolation-xenomorph-rig-99bc6d43f5c94c7a9ee9f7198b935561)
